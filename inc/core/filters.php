...
function thds_template_include( $template ) {
	...
	if ( thds_is_author() ) {
		...
		// Subject archive.
	} elseif ( thds_is_subject() ) {
		$templates[] = 'theme-subject.php';

		// Feature archive.
	} elseif ( thds_is_feature() ) {
		$templates[] = 'theme-feature.php';

		// Single theme.
	} elseif ( thds_is_single_theme() ) {
...
function thds_pre_get_posts( $query ) {
	...
	if ( ( ! is_admin() && $query->is_main_query() && thds_is_archive() ) || is_search() ) {
...
function thds_get_the_archive_description( $desc ) {
	...
	} elseif ( thds_is_theme_archive() && ! $desc ) {
