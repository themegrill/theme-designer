...
/**
 * Prints the happy customers count.
 *
 * @since  1.0.0
 * @access public
 * @param  int     $theme_id
 * @return void
 */
function thds_theme_happy_customers_count( $theme_id = 0 ) {

	echo number_format_i18n( absint( thds_get_theme_happy_customers_count( $theme_id ) ) );
}

/**
 * Returns the happy customers count.
 *
 * @since  1.0.0
 * @access public
 * @param  int     $theme_id
 * @return int
 */
function thds_get_theme_happy_customers_count( $theme_id = 0 ) {

	$theme_id = thds_get_theme_id( $theme_id );

	return apply_filters( 'thds_get_theme_happy_customers_count', absint( thds_get_theme_meta( $theme_id, 'install_count' ) ), $theme_id );
}

/**
 * Returns the star ratings.
 *
 * @since  1.0.0
 * @access public
 * @param  int     $theme_id
...
function thds_get_theme_star_rating( $theme_id = 0 ) {

	$theme_id = thds_get_theme_id( $theme_id );
	return apply_filters( 'thds_get_theme_star_rating', floatval( thds_get_theme_meta( $theme_id, 'rating' ) ), $theme_id );
}
...
function thds_theme_install_count( $theme_id = 0 ) {
	...
	printf( __( '%s+', 'theme-designer' ), number_format_i18n( absint( thds_get_theme_install_count( $theme_id ) ) ) );
}
...
/**
 * Returns the popular theme name.
 *
 * @since  1.0.0
 * @access public
 ...
 * @return int
 */
function thds_get_theme_popular_title( $post_id = 0 ) {

	$selected_theme_id = get_post_meta( $post_id, 'selected_theme', true );

	$selected_theme_title = '';
	if ( $selected_theme_id ) {
		$selected_theme = get_post( $selected_theme_id );
		if ( $selected_theme ) {
			$selected_theme_title = $selected_theme->post_title;
		}
	}

	return $selected_theme_title;
}
