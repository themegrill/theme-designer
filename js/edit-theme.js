jQuery(document).ready(function ($) {
  ...
  jQuery("a.thds-edit-sticky").click(function (j) {
    j.preventDefault();
  ...
  jQuery("a.thds-save-sticky").click(function (j) {
    j.preventDefault();
  ...
  jQuery("a.thds-cancel-sticky").click(function (j) {
    j.preventDefault();
  ...
  jQuery(sticky_checkbox).change(function () {
    jQuery("strong.thds-sticky-status").text(
      jQuery(sticky_checkbox).prop("checked")
        ? thds_i18n.label_sticky
        : thds_i18n.label_not_sticky
    );
  });
  ...
  jQuery(".thds-which-tab").text(
    jQuery(".thds-fields-nav :first-child a").text()
  );

  // When a tab nav item is clicked.
  jQuery(".thds-fields-nav li a").click(function (j) {
    ...
    jQuery(this)
      .parents(".thds-fields-manager")
      .find(".thds-fields-section")
      .hide();
  ...
  /* ====== Repeater Fields ====== */
  var $wrapper = $(".repeater-wrapper");
  var $addField = $wrapper.find(".add-field");
  var $removeField = $wrapper.find(".remove-field");
  var count = $wrapper.find(".repeater-container").children().length;
  var $fileName = $wrapper.data("name");

  $addField.on("click", function (e) {
    e.preventDefault();
    count++;
    $wrapper.find(".repeater-container").append(
      $(`
	<div class="repeater-field">
	 <div class="repeater-field-title">
      Title
      <input type="text" name="${$fileName}[${count}][title][]"  /> 
    </div>
    <br />
    <div class="repeater-field-key">
      Key
      <input type="text" name="${$fileName}[${count}][key][]" /> 
    </div>
    <br />
    <div class="repeater-field-value">
      Value
      <input type="text" name="${$fileName}[${count}][value][]" /> 
    </div>
    <br />
