...
final class THDS_Theme_Edit {
	...
	public function load() {
		...
		add_action(
			...
			function ( $post_id ) {
				...
				if ( ! isset( $_POST['thds_blog_meta_box_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['thds_blog_meta_box_nonce'] ) ), 'save_selected_theme' ) ) {
					return;
				}
				...
				if ( isset( $_POST['selected_theme'] ) ) {
					update_post_meta( $post_id, 'selected_theme', sanitize_text_field( $_POST['selected_theme'] ) );
				}

				$theme_type = isset( $_POST['theme_type'] ) ? $_POST['theme_type'] : 0;
				$theme_type = $theme_type ? $theme_type : 0;
				update_post_meta( $post_id, 'theme_type', $theme_type );
			}
		);
	}
	...
	public function add_meta_boxes( $post_type ) {
		...
		add_meta_box(
			...
			function ( $post ) {
				...
				wp_nonce_field( 'save_selected_theme', 'thds_blog_meta_box_nonce' );

				$selected_theme = get_post_meta( $post->ID, 'selected_theme', true );
				...
				echo '<label for="selected_theme">Select a theme:</label>';
				echo '<select name="selected_theme" id="selected_theme">';
				echo '<option value="">Select a theme...</option>';
				foreach ( $themes as $theme ) {
					$selected = selected( $selected_theme, $theme->ID, false );
					echo '<option value="' . esc_attr( $theme->ID ) . ' " ' . esc_attr( $selected ) . ' >' . esc_html( $theme->post_title ) . '</option>';
				}
				echo '</select>';
			},
			null,
		...
		add_meta_box(
			...
			function ( $post ) {
				...
				$theme_type = get_post_meta( $post->ID, 'theme_type', true );

				$post = get_post( $post->ID );

				echo '<input type="checkbox" value="1" name="theme_type" id="theme_type" ' . checked( $theme_type, 1, false ) . '/>';
				echo '<label for="theme_type">Is Premium?</label>';
			},
			null,
			'side'
		);
	...
	public function add_help_tabs() {
		...
		// Details: Compare Table help tab.
		$screen->add_help_tab(
			array(
				'id'       => 'details_compare_table',
				'title'    => esc_html__( 'Details: Compare Table', 'theme-designer' ),
				'callback' => array( $this, 'help_tab_details_compare_table' ),
			)
		);
	...
	public function help_tab_details_general() {
		...
		<ul>
			...
			<li><?php _e( '<strong>Theme Type:</strong> Type of the theme.', 'theme-designer' ); ?></li>
	...
	/**
	 * Displays the compare table details help tab.
	 *
	 * @since  1.0.0
	 * @access public
	 * @return void
	 */
	public function help_tab_details_compare_table() {
		?>

		<p>
			<?php esc_html_e( 'The compare table section is for table to compare the features with our best themes.', 'theme-designer' ); ?>
		</p>

		<ul>
			<li><?php _e( '<strong>Customer Review:</strong> Compare Customer Review to get the best rated theme.', 'theme-designer' ); ?></li>
			<li><?php _e( '<strong>Price:</strong> Get Price comparison between two themes.', 'theme-designer' ); ?></li>
			<li><?php _e( '<strong>Total Downloads:</strong> Compare the count of the theme Total Downloads.', 'theme-designer' ); ?></li>
			<li><?php _e( '<strong>Total Demos:</strong> Compare the Total number of Demos available for both theme.', 'theme-designer' ); ?></li>
			<li><?php _e( '<strong>Happy Customers:</strong> Get the total number of Happy Customers for both theme.', 'theme-designer' ); ?></li>
		</ul>
		<?php
	}
