...
function thds_admin_register_scripts() {
	...
	wp_register_script( 'thds-edit-theme', thds_plugin()->js_uri . 'edit-theme.js', array( 'jquery' ), time(), true );
...
function thds_theme_details_register( $manager ) {
	...
	$manager->register_section(
		'compare-meta',
		array(
			'label' => esc_html__( 'Compare Meta', 'theme-designer' ),
			'icon'  => 'dashicons-edit',
		)
	);
	...
	$compare_field_args = array(
		'section' => 'compare-meta',
		'type'    => 'repeater',
		'attr'    => array(
			'name' => 'compare',
		),
	...
	$manager->register_control( 'compare_fields', $compare_field_args );
	...
	$manager->register_setting( 'compare_fields', array( 'sanitize_callback' => '' ) );
